    ...
    <section class="section dashboard">
      <div class="row">
        ...
        <form method="post" action="{{route('raiserequest')}}">
            @csrf
                        <div class="card-body">
                          <h5 class="card-title">Recived samples <span></span></h5>
                          @if($samples->count() == 0)
No data found
                          @else
                            ...
                            <div>
                            <input type="checkbox" name="sample[]" value="{{$sample->id}}">
                        
                            <label> Id : {{$sample->id}} || Custumer Name : {{$sample->name ?? ''}} || Mobile : {{$sample->mobile}} || TestDetails : @foreach($sample->testdetails as $test) {{$test->name->name ?? ''}},  @endforeach || @foreach($sample->packagedetails as $details) {{$details->pc->package}}, @endforeach  || Boy details : {{$sample->boydetails->name ?? '' }} <label>
</div>
                                @endforeach
                                <br>
